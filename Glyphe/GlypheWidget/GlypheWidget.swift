...
struct RandomIconsProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> RandomIconsEntry {
        RandomIconsEntry(date: Date(), icon1: UIImage(), icon2: UIImage(), icon3: UIImage(), icon4: UIImage())
    }

    func getSnapshot(in context: Context, completion: @escaping (RandomIconsEntry) -> ()) {
        let entry = RandomIconsEntry(date: Date(), icon1: UIImage(), icon2: UIImage(), icon3: UIImage(), icon4: UIImage())
        completion(entry)
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        ...
        let entry = RandomIconsEntry(date: Date(), icon1: randomIcons.0, icon2: randomIcons.1, icon3: randomIcons.2, icon4: randomIcons.3)
    ...
    func fetchRandomIcons() -> (UIImage, UIImage, UIImage, UIImage) {
        ...
        let randomIndexes = iconNames.indices.shuffled().prefix(4)
        let icons = randomIndexes.map { UIImage(named: iconNames[$0]) ?? UIImage() }
        return (icons[0], icons[1], icons[2], icons[3])
    }
}
